...
export class ChatWebSocketManager {
  ...
  async connect(token: string, userId: string, userType: string, roomId: number, lastMessageId?: number): Promise<boolean> {
    return new Promise(async (resolve, reject) => {
      try {
        // 기존 연결이 있으면 완전히 해제될 때까지 대기
        console.log('기존 연결 상태 체크:', {
        ...
        this.socket.onmessage = (event) => {
          try {
            ...
            // 모든 소켓 메시지를 상세히 로깅
            console.log('🔵 [WebSocket 수신] 전체 메시지:', data);
            console.log('🔵 [WebSocket 수신] 요약:', {
              type: data.type,
            ...
            if (data.type === 'messages_read_notification') {
              console.log('📖 [특별 로그] messages_read_notification 수신!', {
                fullData: data,
                timestamp: new Date().toISOString()
              });
            }
            ...
            if (data.type === 'messages_read') {
              console.log('📖 [특별 로그] messages_read 수신!', {
                roomId: data.roomId,
                readBy: data.readBy,
                count: data.count,
                fullData: data,
            ...
            // 이벤트 타입별 핸들러만 호출 (중복 방지)
            this.emitEvent(data.type, data);
          } catch (error) {
            console.error('❌ WebSocket 메시지 파싱 오류:', error, 'Raw data:', event.data);
          }
        };
  ...
  sendTypingStatus(roomId: number, isTyping: boolean, userId: string, userType: string): boolean {
    if (!this.socket || this.socket.readyState !== WebSocket.OPEN) {
      console.log('⚠️ [타이핑] WebSocket 연결되지 않음 - 타이핑 상태 전송 불가');
      return false;
    }
