...
LOG_FILE="/var/log/doberman-monitoring.log"
ERROR_LOG="/var/log/doberman-monitoring-error.log"
...
if [ "$1" = "start" ]; then
    main &
    echo $! > /var/run/doberman-monitoring.pid
    log_message "INFO" "모니터링 데몬 시작됨 (PID: $!)"
elif [ "$1" = "stop" ]; then
    if [ -f /var/run/doberman-monitoring.pid ]; then
        kill $(cat /var/run/doberman-monitoring.pid)
        rm /var/run/doberman-monitoring.pid
