...
export const useChatLogic = (chatPartner?: CustomerRequestAnswer, requestId?: number) => {
    ...
    (userId) => {
      ...
      if (userId !== chatAuth.userId) {
        ...
        if (unreadMyMessages.length > 0) {
          const latestMessage = unreadMyMessages[unreadMyMessages.length - 1];
          updateMessage(latestMessage.messageId, { isRead: true });
          
          // 최근 메시지가 읽음 처리되면 이전 메시지들도 자동으로 읽음 처리됨
          setTimeout(() => {
            markPreviousMessagesAsRead(latestMessage.messageId);
          }, 100);
        }
      }
    },
  ...
  const markPreviousMessagesAsRead = useCallback((lastReadMessageId: number) => {
    ...
    if (lastReadIndex === -1) {
      console.log('마지막 읽은 메시지를 찾을 수 없습니다:', lastReadMessageId);
      return;
    }

    // 마지막 읽은 메시지 이전의 모든 메시지를 읽음 처리
    const messagesToMarkAsRead = sortedMessages.slice(0, lastReadIndex + 1).filter(msg => !msg.isRead);
    
    console.log(`${messagesToMarkAsRead.length}개의 메시지를 읽음 처리합니다.`);
