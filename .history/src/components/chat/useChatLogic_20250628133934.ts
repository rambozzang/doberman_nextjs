...
export const useChatLogic = (chatPartner?: CustomerRequestAnswer, requestId?: number) => {
  ...
  const { 
    ...
    // 메시지 전송 완료 시 - 서버에서 확인된 메시지로 교체
    (sentMessage) => {
      console.log('메시지 전송 완료:', sentMessage);
      
      // 임시 메시지를 서버에서 확인된 메시지로 교체
      // 임시 messageId는 Date.now()로 생성했으므로 찾아서 교체
      const tempMessageIndex = messages.findIndex(msg => 
        ...
        msg.messageId > 1000000000000 // Date.now() 기반 임시 ID
      );
      
      if (tempMessageIndex !== -1) {
        // 임시 메시지 제거하고 서버 메시지 추가
        removeMessage(messages[tempMessageIndex].messageId);
        
        // 서버에서 온 메시지는 항상 isRead: false로 설정 (내가 보낸 메시지)
