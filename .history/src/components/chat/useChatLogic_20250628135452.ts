...
export const useChatLogic = (chatPartner?: CustomerRequestAnswer, requestId?: number) => {
  ...
  const { 
    ...
    (sentMessage) => {
      ...
      // 서버에서 메시지 내용이 없는 경우 (일반적) - 기존 임시 메시지 찾아서 업데이트
      if (!sentMessage.message || !sentMessage.message.trim()) {
        // 임시 메시지 ID로 찾아서 서버 ID와 isRead 상태로 업데이트
        ...
        if (tempMessages.length > 0) {
          ...
          const updatedMessage = {
            ...
            messageId: sentMessage.messageId,
            isRead: sentMessage.isRead,
            createdAt: sentMessage.createdAt || latestTempMessage.createdAt
          };
          
          addMessage(updatedMessage);
        }
      } else {
        // 서버에서 메시지 내용이 있는 경우 - 그대로 추가
