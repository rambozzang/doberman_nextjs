...
export const ChatModal: React.FC<ChatModalProps> = ({
  ...
  // API 메시지를 UI용 메시지로 변환 (빈 메시지 필터링)
  const convertedMessages = useMemo(() => {
    return messages
      .filter(apiMessage => apiMessage.message && apiMessage.message.trim() !== '') // 빈 메시지 필터링
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-gradient-to-b from-slate-900/50 to-slate-800/50 backdrop-blur-sm">
          {isLoading && convertedMessages.length === 0 ? (
            ...
            convertedMessages.map((message, index) => {
              // 현재 사용자가 보낸 메시지인지 확인
              const isMyMessage = message.senderId === chatAuth.userId;
              
              return (
                <div
                  key={message.id}
                  ref={(el) => {
                    // 상대방 메시지에만 Intersection Observer 적용
                    if (el && !isMyMessage && observeMessage) {
                      // 메시지 요소에 필요한 데이터 속성 설정
                  ...
                  } animate-in slide-in-from-bottom-2 duration-300 message-item ${
                    isMyMessage ? 'own-message' : ''
                  }`}
