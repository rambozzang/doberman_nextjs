...
export const useChatLogic = (chatPartner?: CustomerRequestAnswer, requestId?: number) => {
  ...
  const { chatRooms, createChatRoom, findChatRoomByRequestId, updateLastMessage, updateUnreadCount } = useChatRooms();
  const { 
    messages, 
    isLoading, 
    addMessage, 
    ...
    if (chatPartner?.user?.id && requestId) {
      try {
        ...
        if (existingRoomId) {
          ...
          const roomId = await createChatRoom(requestId, chatPartner.user.id.toString());
