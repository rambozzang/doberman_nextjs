...
export const useChatLogic = (chatPartner?: CustomerRequestAnswer, requestId?: number) => {
  ...
  const { createChatRoom, findChatRoomByRequestId, findChatRoomByUsers, updateLastMessage, updateUnreadCount } = useChatRooms();
  const { 
    messages, 
    isLoading, 
    addMessage, 
  ...
  const { 
    ...
    // 메시지 전송 완료 시 (사용하지 않음 - 즉시 UI 업데이트 사용)
    undefined,
    // 타이핑 상태 변경 시
    (isTyping, userId) => {
  ...
  const openChat = useCallback(async () => {
    ...
    try {
      ...
      if (existingRoomId) {
        ...
        console.log('- chatPartner.userId:', chatPartner.userId);
        console.log('- chatPartner.answerId:', chatPartner.answerId);
        ...
        console.log('- 사용할 expertId:', expertId);
        ...
        try {
          ...
          // 422 오류인 경우 이미 채팅방이 존재할 가능성이 높음
          if (createError?.response?.status === 422) {
            console.log('422 오류 발생 - 채팅방이 이미 존재할 수 있음. 다시 조회 시도...');
            
            // 잠시 대기 후 다시 조회 시도
            await new Promise(resolve => setTimeout(resolve, 1000));
            
            // 1. requestId로 조회 시도
            let retryRoomId = await findChatRoomByRequestId(requestId);
            
            // 2. requestId로 조회 실패 시 customerId + userId로 조회 시도
            if (!retryRoomId) {
              console.log('requestId 조회 실패, customerId + userId로 재시도...');
              retryRoomId = await findChatRoomByUsers(requestId, expertId.toString());
            }
            
            if (retryRoomId) {
              setCurrentRoomId(retryRoomId);
              console.log('재조회로 기존 채팅방 찾음:', retryRoomId);
            } else {
              throw new Error('채팅방 생성에 실패했습니다. (422 오류 - 기존 채팅방도 찾을 수 없음)');
            }
          } else {
            throw createError;
          }
