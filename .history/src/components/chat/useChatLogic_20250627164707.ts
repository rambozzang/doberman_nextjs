...
export const useChatLogic = (chatPartner?: CustomerRequestAnswer, requestId?: number) => {
  ...
  const { 
    ...
    (message) => {
      console.log('🔵 [message_sent] 메시지 전송 확인:', {
      ...
      if (tempMessages.length > 0) {
        ...
        console.log('🔄 임시 메시지를 실제 메시지로 교체:', latestTempMessage.messageId, '->', message.messageId, 'isRead:', message.isRead);
        ...
        console.log('⚠️ 교체할 임시 메시지를 찾을 수 없습니다. 새 메시지로 추가합니다.');
        addMessage({
          ...message,
          isRead: message.isRead
        });
  ...
  const getFormattedMessages = useCallback((): ChatMessage[] => {
    ...
    const formattedMessages = sortedMessages
      .filter(msg => {
        ...
        const hasFilePath = msg.filePath && msg.filePath.trim().length > 0;
        
        if (!hasMessage && !hasFilePath) {
          console.log('🗑️ [메시지 필터링] 빈 메시지 제외:', {
      ...
      .map(msg => ({
        ...
        senderType: (msg.senderType === 'WEB' ? 'customer' : 'expert') as 'customer' | 'expert',
