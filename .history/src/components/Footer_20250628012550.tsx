      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="py-4 sm:py-6 lg:py-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-3 sm:gap-4 lg:gap-6">
            {/* Company Info */}
            <div className="md:col-span-2 lg:col-span-4 space-y-3 sm:space-y-4 lg:space-y-4">
              <motion.div
                ...
                viewport={{ once: true }}
              >
                <Link href="/" className="flex items-center space-x-2 mb-2 sm:mb-3 lg:mb-3">
                  <Image src="/logo.png" alt="logo" width={24} height={24} className="sm:w-7 sm:h-7 lg:w-7 lg:h-7" />
                  <span className="text-base sm:text-lg lg:text-lg font-bold text-foreground">{logo}</span>
                </Link>
                <p className="text-muted-foreground text-xs sm:text-sm lg:text-sm leading-relaxed pr-4 line-clamp-3">
                  {description}
                </p>
              </motion.div>
                    ...
                    <ul className="space-y-1 sm:space-y-1.5 lg:space-y-1.5">
                      {section.links.map((link) => (
                        <li key={link.title}>
                          <Link
                            href={link.href}
                            className="text-xs sm:text-sm lg:text-sm text-muted-foreground hover:text-foreground transition-colors block py-0.5 lg:py-0.5 touch-target"
                          >
                            {link.title}
                          </Link>
                        </li>
                ...
                >
                  ...
                  <form onSubmit={handleNewsletterSubmit} className="space-y-2 lg:space-y-2">
                    <input
                      type="email"
                      placeholder="이메일 주소"
                      className="w-full px-2.5 py-1.5 lg:px-3 lg:py-2 text-xs lg:text-sm bg-background border border-border rounded-md focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent"
                      required
                    />
                    <button 
                      type="submit" 
                      className="w-full px-2.5 py-1.5 lg:px-3 lg:py-2 text-xs lg:text-sm bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-colors"
                    >
                      구독하기
                    </button>
                  </form>
        ...
        <motion.div
          ...
          viewport={{ once: true }}
          className="py-4 lg:py-4 border-t border-border"
        >
