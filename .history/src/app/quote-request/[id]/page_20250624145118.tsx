            ...
            <div className="space-y-6">
              ...
              <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-6">
                ...
                <div className="text-center py-4 mb-4">
                  ...
                  <div className="text-2xl font-bold text-white mb-1">{customerRequest.answerCount}</div>
                  <div className="text-slate-400 text-sm">개의 답변</div>
                </div>
                ...
                <div className="space-y-3 max-h-96 overflow-y-auto">
                  {isAnswersLoading ? (
                    <div className="text-center py-4">
                      <RefreshCwIcon className="w-5 h-5 text-blue-400 animate-spin mx-auto mb-2" />
                      <div className="text-slate-400 text-sm">답변을 불러오는 중...</div>
                    </div>
                  ) : answers.length > 0 ? (
                    answers.map((answer) => (
                      <div key={answer.id} className="bg-slate-800/30 rounded-lg p-4 border border-slate-700/50">
                        <div className="flex items-start justify-between mb-2">
                          <div className="flex-1">
                            ...
                            <div className="text-xs text-slate-400 mb-2">
                              {answer.user?.userName || answer.webCustomer?.customerName || "익명"} • {answer.createdDt ? formatDateTime(answer.createdDt) : ""}
                            </div>
                          </div>
                          {answer.cost && (
                        ...
                        {answer.answerBody && (
                          <div className="text-slate-300 text-sm">
                            {answer.answerBody.length > 100 
                              ? `${answer.answerBody.substring(0, 100)}...` 
                              : answer.answerBody
                            }
                          </div>
                        )}
                    ...
                    <div className="text-center py-4">
                      <MessageCircleIcon className="w-8 h-8 text-slate-500 mx-auto mb-2" />
                      <div className="text-slate-400 text-sm">아직 답변이 없습니다</div>
                    </div>
                  )}
                </div>
              </div>
