  ...
  return (
    <div className="flex flex-col min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
      ...
      {showChatModal && chatPartner && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-gradient-to-br from-slate-800/95 via-slate-700/95 to-slate-800/95 backdrop-blur-xl border border-white/10 rounded-2xl p-6 max-w-2xl w-full max-h-[80vh] shadow-2xl animate-in fade-in-0 zoom-in-95 duration-300">
            {/* 모달 헤더 */}
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-3">
                <div className="flex items-center justify-center w-12 h-12 bg-gradient-to-br from-blue-500/20 to-purple-500/20 rounded-full">
                  <MessageSquareIcon className="w-6 h-6 text-blue-400" />
                </div>
                <div>
                  <h3 className="text-xl font-bold text-white">채팅</h3>
                  <p className="text-slate-400 text-sm">
                    {chatPartner.user?.userName || chatPartner.webCustomer?.customerName || '전문가'}와 대화
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="flex flex-col h-[500px]">
              {/* 채팅 상대방 정보 */}
              <div className="flex items-center gap-3 p-4 bg-gradient-to-r from-slate-900/50 to-slate-800/50 rounded-lg mb-4 border border-slate-600/30">
                <div className="flex items-center justify-center w-12 h-12 bg-gradient-to-br from-blue-500/20 to-purple-500/20 rounded-full">
                ...
                {chatPartner.cost && (
                  <div className="text-right">
                    <p className="text-xs text-slate-400">채택 금액</p>
              ...
              <div 
                ref={chatMessagesRef}
                className="flex-1 overflow-y-auto space-y-4 p-4 bg-gradient-to-b from-slate-900/30 to-slate-800/30 rounded-lg border border-slate-700/30 min-h-0"
              >
                  ...
                  >
                    <div className={`max-w-[75%] ${
                      message.senderType === 'customer' 
                        ...
                        : 'bg-gradient-to-r from-slate-600/90 to-slate-700/90'
                      ...
                      <div className="flex items-center justify-end gap-2 mt-2">
                        ...
                        {message.senderType === 'customer' && (
                          <div className="flex items-center">
                            {message.isRead ? (
                              <CheckCheckIcon className="w-4 h-4 text-blue-300" />
                            ) : (
              ...
              {/* 메시지 입력 영역 */}
              <div className="flex items-end gap-3 mt-4 p-3 bg-gradient-to-r from-slate-800/50 to-slate-700/50 rounded-lg border border-slate-600/30">
                <textarea
                  value={newMessage}
                  ...
                  rows={2}
                  className="flex-1 p-3 bg-slate-700/50 border border-slate-600/30 rounded-lg text-white placeholder-slate-400 focus:outline-none focus:border-blue-400/50 transition-colors resize-none"
                />
                <button
                  onClick={handleSendMessage}
                  disabled={!newMessage.trim()}
                  className="p-3 bg-gradient-to-r from-blue-500/20 to-blue-600/20 hover:from-blue-500/30 hover:to-blue-600/30 disabled:opacity-50 disabled:cursor-not-allowed border border-blue-500/30 rounded-lg transition-all duration-200 hover:scale-105"
                >
