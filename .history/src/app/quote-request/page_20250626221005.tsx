...
export default function QuoteRequestPage() {
  ...
  const nextStep = () => {
    ...
    // 로그인된 경우 6단계(지역선택) 후 바로 완료 단계로 이동
    if (isLoggedIn && currentStep === 5) {
      setCurrentStep(7); // 완료 단계로 바로 이동
      window.scrollTo(0, 0);
      return;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // 마지막 단계(완료)에서는 아무것도 하지 않음
    if (currentStep === steps.length - 1) {
      return;
    }
    
    // 견적 요청 단계가 아니면 다음 단계로 이동
    if ((isLoggedIn && currentStep < 5) || (!isLoggedIn && currentStep < 6)) {
      nextStep();
      return;
    }
    
    // 마지막 견적 요청 단계에서 다음 단계(완료 단계)로 이동
    nextStep();
  };
              ...
              <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-3 sm:p-4 md:p-6 lg:p-8 shadow-2xl">
                <form onSubmit={handleSubmit}>
                  <div className="mb-3 sm:mb-6 md:mb-8 lg:mb-10">
                    <div className="flex flex-col sm:flex-row items-start sm:items-center mb-2 sm:mb-4 md:mb-6">
                      <div className="relative mb-1 sm:mb-0 sm:mr-4">
                        <div className="w-6 h-6 sm:w-10 sm:h-10 md:w-12 md:h-12 bg-gradient-to-br from-blue-500 via-purple-500 to-indigo-600 rounded-lg sm:rounded-xl flex items-center justify-center shadow-lg shadow-blue-500/25">
                          {(() => {
                            const Icon = steps[currentStep].icon;
                            return <Icon className="w-3 h-3 sm:w-5 sm:h-5 md:w-6 md:h-6 text-white" />;
                          })()}
                        </div>
                      </div>
                      <div className="flex-1 w-full">
                        <div className="flex flex-col sm:flex-row sm:items-center mb-0.5">
                          <h3 className="text-sm sm:text-lg md:text-xl font-bold text-white mb-0.5 sm:mb-0 sm:mr-3">{steps[currentStep].title}</h3>
                          <div className="px-2 py-0.5 bg-blue-500/20 border border-blue-400/30 rounded-full self-start">
                            <span className="text-xs font-medium text-blue-300">
                              {currentStep + 1}/{steps.length}단계
                            </span>
                          </div>
                        </div>
                        <p className="text-xs sm:text-sm text-slate-400">{steps[currentStep].description}</p>
                      </div>
                    </div>
                  </div>
