...
export default function MyQuoteRequestsPage() {
  ...
  }, [shouldRender, user, currentPage, pageSize, statusFilter]);
              ...
              <div className="space-y-4">
                {filteredRequests.map((request) => {
                  ...
                  return (
                    <div
                      key={request.id}
                      className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-lg p-2 hover:border-white/20 hover:scale-[1.005] hover:-translate-y-0.5 transition-all duration-300"
                    >
                      <div className="grid grid-cols-1 lg:grid-cols-4 gap-2">
                        ...
                        <div className="lg:col-span-3">
                          ...
                          <div className="flex items-start justify-between mb-1">
                            <div className="flex-1">
                              <div className="flex items-center gap-1.5 mb-0.5">
                                ...
                                <div className={`flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${statusConfig.bgColor} ${statusConfig.textColor}`}>
                          ...
                          {/* 상세 정보 */}
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-1 mb-1 text-xs">
                            <div className="flex items-center text-slate-300">
                              <BuildingIcon className="w-2.5 h-2.5 mr-1 text-slate-400" />
                            ...
                            <div className="flex items-center text-slate-300">
                              <MapPinIcon className="w-2.5 h-2.5 mr-1 text-slate-400" />
                              <span className="truncate">{request.region}</span>
                            </div>
                            
                            <div className="flex items-center text-slate-300">
                              <CalendarIcon className="w-2.5 h-2.5 mr-1 text-slate-400" />
                          ...
                          <div className="flex items-center gap-2">
                            {request.wallpaper && (
                              <span className="px-1.5 py-0.5 bg-slate-700/50 text-xs text-slate-300 rounded text-nowrap">
                                {removeBrackets(request.wallpaper)}
                              </span>
                            )}
                            {request.specialInfo && (
                              <p className="text-xs text-slate-400 truncate flex-1">{request.specialInfo}</p>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="flex lg:flex-col gap-2">
                          {/* 견적 정보 */}
                          <div className="bg-slate-800/30 rounded-md p-1.5 flex-1 lg:flex-none">
                            <div className="text-center">
                              <div className="text-base font-bold text-blue-400">{request.answerCount}</div>
                              <div className="text-xs text-slate-400">답변</div>
                            </div>
                          </div>
                          ...
                          <div className="bg-slate-800/30 rounded-md p-1.5 flex-1 lg:flex-none">
                            <div className="text-center">
                              <div className="text-xs text-slate-400">고객</div>
                              <div className="text-xs text-white font-medium truncate">{request.customerName}</div>
                            </div>
                          </div>
                          ...
                          <div className="flex lg:flex-col gap-1">
                            <Link 
                              ...
                              className="flex items-center justify-center gap-1 px-2 py-1 bg-blue-500/20 hover:bg-blue-500/30 border border-blue-500/30 rounded transition-all text-blue-400 hover:text-blue-300 text-xs whitespace-nowrap"
                            >
                            ...
                            {request.status === '검토중' && (
                              <>
                                <button className="flex items-center justify-center gap-1 px-2 py-1 bg-slate-700/50 hover:bg-slate-700/70 border border-slate-600 rounded transition-all text-slate-300 hover:text-white text-xs whitespace-nowrap">
                                  <EditIcon className="w-2.5 h-2.5" />
                                  수정
                                </button>
                                <button 
                                  onClick={() => handleDelete(request.id)}
                                  className="flex items-center justify-center gap-1 px-2 py-1 bg-red-500/20 hover:bg-red-500/30 border border-red-500/30 rounded transition-all text-red-400 hover:text-red-300 text-xs whitespace-nowrap"
                                >
