    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        setStats({
          ...
          inProgress: statsData.adoptedCount, // 채택 성공을 진행중으로 매핑
          completed: statsData.completedCount,
          canceled: statsData.canceledCount
        });
        console.log("통계 데이터 설정 완료:", statsData);
      } else {
        console.log("통계 API 응답 실패:", response);
        toast.error(response.error || "통계 데이터를 불러오는데 실패했습니다.");
      }
    } catch (error) {
  ...
  const loadCustomerRequests = useCallback(async (
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        // 통계 정보 설정 (기존 방식 유지 - 별도 통계 API로 대체 예정)
        ...
        setStats({
          ...
          canceled: contentArray.filter(r => r.status === '취소').length
        });
