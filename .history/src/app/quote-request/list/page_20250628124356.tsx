...
export default function QuoteRequestListPage() {
  ...
  useEffect(() => {
    ...
    // localStorage 변화 감지 (다른 탭에서 로그인/로그아웃)
    ...
    // 포커스 시 사용자 정보 재확인 (같은 탭에서 로그인 후 돌아왔을 때)
    const handleFocus = () => {
      updateUserInfo();
    };

    // 주기적으로 사용자 정보 확인 (실시간 반영을 위해)
                    ...
                    >
                      ...
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex-1 min-w-0">
                          ...
                          {/* 임시 디버그 정보 */}
                          <div className="text-xs text-red-400 bg-red-500/10 p-2 rounded mb-2 border border-red-500/20">
                            <div>현재 사용자 이메일: {currentUser?.customerEmail || 'null'}</div>
                            <div>견적 고객 이메일: {request.customerEmail || 'null'}</div>
                            <div>현재 사용자 ID: {currentUser?.customerId || 'null'}</div>
                            <div>견적 고객 ID: {(request as any).customerId || request.webCustomerId || 'null'}</div>
                            <div>로그인 상태: {AuthManager.isLoggedIn() ? 'TRUE' : 'FALSE'}</div>
                            <div>토큰 존재: {AuthManager.getToken() ? 'TRUE' : 'FALSE'}</div>
                            <div>localStorage 사용자정보: {typeof window !== 'undefined' ? localStorage.getItem('user_info') || 'null' : 'SSR'}</div>
                            <div>일치 여부: {canAccessRequest(request) ? 'TRUE' : 'FALSE'}</div>
                          </div>
