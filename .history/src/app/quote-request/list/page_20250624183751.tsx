...
export default function QuoteRequestListPage() {
  ...
  const loadStatsAndFilters = async () => {
    try {
      const response = await CustomerRequestService.searchRequests({ page: 0, size: 1000 }); // 전체 데이터 조회
      if (response.success && response.data) {
      ...
      console.error("통계 정보 및 필터 옵션 로딩 오류:", error);
    }
  };

  // 초기 데이터 로딩 및 필터 옵션 로딩
  useEffect(() => {
    // 초기 로딩: 전체 데이터로 필터 옵션 가져오기
    loadCustomerRequests(0, pageSize, "", "all", "all", sortBy, sortOrder);
    loadStatsAndFilters();
  }, []);

  // 검색/필터/정렬 변경 시 데이터 다시 로딩
  useEffect(() => {
  ...
  }, [searchTerm, statusFilter, regionFilter, sortBy, sortOrder, pageSize]);
