...
export default function QuoteRequestListPage() {
    ...
    const initData = async () => {
      ...
      try {
        ...
        if (isMounted && statsResponse.success && statsResponse.data) {
          ...
          console.log("전체 요청 수:", allRequests?.length);
          
          // 통계 정보 설정
          setStats({
            total: statsResponse.data.totalCount || 0,
            reviewing: allRequests?.filter(r => r.status === '검토중').length || 0,
            inProgress: allRequests?.filter(r => r.status === '진행중').length || 0,
            completed: allRequests?.filter(r => r.status === '채택 성공').length || 0
          });
          ...
          console.log("통계 데이터 로딩 실패:", statsResponse);
        }
        ...
        console.log("페이지 데이터 응답:", response);
        ...
        if (isMounted) {
          if (response.success && response.data) {
            ...
            console.log("페이지 데이터 로딩 실패:", response);
            setCustomerRequests([]);
            setCurrentPage(0);
            setTotalPages(0);
            setTotalCount(0);
  ...
  }, [pageSize, sortBy, sortOrder]); // 의존성 배열에 필요한 값들 추가
