  ...
  const loadCustomerRequests = async (
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        // 첫 로딩시에만 필터 옵션 업데이트 (전체 데이터 기반)
        if (page === 0 && !searchKeyword && status === "all" && region === "all") {
          const uniqueStatuses = [...new Set(response.data.content.map(req => req.status))];
          const uniqueRegions = [...new Set(response.data.content.map(req => req.region))];
