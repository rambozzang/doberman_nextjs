      ...
      try {
        ...
        if (isMounted) {
          ...
          if (response && response.success === true && response.data) {
            ...
            console.log("content 길이:", contentData.length);
            
            setCustomerRequests(contentData);
            setCurrentPage(response.data.currentPage || 0);
            setTotalPages(response.data.totalPages || 0);
            setTotalCount(response.data.totalCount || 0);
            setIsLast(response.data.isLast ?? true);
            ...
            setStats({
              ...
              reviewing: contentData.filter(r => r.status === '검토중').length,
              inProgress: contentData.filter(r => r.status === '진행중').length,
              completed: contentData.filter(r => r.status === '채택 성공').length
            });

            // 필터 옵션 설정
            if (contentData.length > 0) {
              const uniqueStatuses = [...new Set(contentData.map(req => req.status))];
              const uniqueRegions = [...new Set(contentData.map(req => req.region))];
              setFilterOptions({
