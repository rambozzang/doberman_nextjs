...
export default function QuoteRequestListPage() {
  ...
  // 전체 통계 정보 및 필터 옵션 로딩 (초기 로딩용)
  const loadInitialData = useCallback(async () => {
    setIsLoading(true);
    try {
      // 통계 및 필터 옵션용 전체 데이터 조회
      const statsResponse = await CustomerRequestService.searchRequests({ page: 0, size: 1000 });
      
      if (statsResponse.success && statsResponse.data) {
        const allRequests = statsResponse.data.content;
        
        // 통계 정보 설정
        setStats({
          total: statsResponse.data.totalCount,
          reviewing: allRequests.filter(r => r.status === '검토중').length,
          inProgress: allRequests.filter(r => r.status === '진행중').length,
          completed: allRequests.filter(r => r.status === '채택 성공').length
        });

        // 필터 옵션 설정
        const uniqueStatuses = [...new Set(allRequests.map(req => req.status))];
        const uniqueRegions = [...new Set(allRequests.map(req => req.region))];
        setFilterOptions({
      ...
      // 실제 페이지 데이터 로딩
      await loadCustomerRequests(0, pageSize, "", "all", "all", sortBy, sortOrder);
      setIsInitialLoaded(true);
    } catch (error) {
      console.error("초기 데이터 로딩 오류:", error);
      setIsLoading(false);
    }
  }, [loadCustomerRequests, pageSize, sortBy, sortOrder]);
  ...
  }, [searchTerm, statusFilter, regionFilter, sortBy, sortOrder, pageSize, isInitialLoaded]);
