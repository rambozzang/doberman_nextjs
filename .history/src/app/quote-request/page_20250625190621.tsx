...
const steps = [
  ...
  { id: 'step-6', title: '고객 정보', fields: ['name', 'phone', 'email', 'password'], icon: User, description: '견적 및 상담을 위한 연락처 정보를 입력해주세요' },
  { id: 'step-7', title: '시공 주소', fields: ['address', 'region'], icon: MapPin, description: '시공 주소와 지역을 입력해주세요' },
...
export default function QuoteRequestPage() {
  ...
  const validateCurrentStep = (): boolean => {
    switch (currentStep) {
      ...
      case 6: // 시공 주소
        return !!(formState.address && formState.region);
      default:
        return true;
    }
  };
                        ...
                        <div>
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 xl:grid-cols-6 gap-3 md:gap-4">
                            {regions.map((region) => (
                              <div
                                key={region.value}
                                className={`group relative items-center justify-center py-2 md:py-2 md:px-2 border-2 rounded-lg md:rounded-xl cursor-pointer transition-all duration-300 min-h-[80px] hover:scale-105 hover:-translate-y-1 ${
