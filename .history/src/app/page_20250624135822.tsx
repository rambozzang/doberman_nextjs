...
import { Plus, Minus, User, Github, Zap, Layers, Play, ExternalLink, ArrowRight, HelpCircle, CheckSquare, MapPin, Star, MapPin as Location, Phone, Award, Clock, Youtube, Calendar } from 'lucide-react';
        ...
        >
          <div className="text-center mb-12">
            <motion.div
              ...
              className="inline-flex items-center gap-2 px-4 py-2 mb-6 bg-gradient-to-r from-red-500/10 to-pink-500/10 backdrop-blur-xl border border-red-500/20 rounded-full"
            >
              <Youtube className="w-5 h-5 text-red-400" />
              <span className="text-sm font-medium text-red-300">실제 시공 영상</span>
            </motion.div>
            ...
            <motion.h2
              ...
              className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-white via-red-100 to-pink-200 mb-4"
            >
              도배 전문가들의
              <br />
            ...
            >
              전문가들의 실제 작업 과정을 통해 품질과 실력을 미리 확인해보세요
            </motion.p>
          </div>
          ...
          {isYoutubeLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {youtubeVideos.slice(0, 8).map((video: YoutubeVideo, index: number) => (
                <motion.div
                  key={video.id}
                  initial={{ opacity: 0, y: 30 }}
                  animate={{ opacity: 1, y: 0 }}
                  ...
                  whileHover={{ scale: 1.05, y: -5 }}
                  className="bg-gradient-to-br from-slate-800/80 to-slate-900/80 backdrop-blur-xl border border-slate-700/50 rounded-xl overflow-hidden group cursor-pointer"
                  onClick={() => window.open(video.url, '_blank')}
                >
                  <div className="relative overflow-hidden">
                    <img
                    ...
                    <div className="absolute inset-0 bg-black/20 group-hover:bg-black/10 transition-colors duration-300 flex items-center justify-center">
                      <div className="w-12 h-12 bg-red-500/80 rounded-full flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
                        <Play className="w-6 h-6 text-white ml-1" />
                      </div>
                    </div>
                  </div>
          ...
          {/* 더 많은 영상 보기 버튼 */}
          {youtubeVideos && youtubeVideos.length > 8 && (
            <div className="text-center mt-8">
              <motion.button
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                className="bg-gradient-to-r from-red-600 to-pink-600 hover:from-red-500 hover:to-pink-500 text-white px-8 py-4 rounded-xl font-semibold transition-all duration-300 flex items-center gap-2 mx-auto"
              >
