...
export const useChatRooms = () => {
  ...
  const { chatAuth, getAuthHeader } = useChatAuth();
  ...
  // 채팅방 생성
  const createChatRoom = useCallback(async (partnerUserId: string, partnerUserType: string) => {
    ...
    try {
      chatApi.setAuthHeader(chatAuth.token);
      const response = await chatApi.createChatRoom(partnerUserId, partnerUserType);
