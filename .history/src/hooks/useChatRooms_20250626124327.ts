...
export const useChatRooms = () => {
  ...
  // requestId와 전문가 ID로 채팅방 조회
  const findChatRoomByRequestAndExpert = useCallback(async (requestId: number, expertId: string) => {
    ...
    try {
      chatApi.setAuthHeader(chatAuth.token);
      const response = await chatApi.findChatRoomByRequestAndExpert(requestId, expertId);
  ...
  // 채팅방 생성
  const createChatRoom = useCallback(async (partnerUserId: string, partnerUserType: string, requestId?: number) => {
    if (!chatAuth.isAuthenticated || !chatAuth.token) {
      throw new Error('인증이 필요합니다.');
    }
    ...
    try {
      chatApi.setAuthHeader(chatAuth.token);
      const response = await chatApi.createChatRoom(partnerUserId, partnerUserType, requestId);

      if (response.success && response.data) {
        console.log('채팅방 생성 성공:', response.data.roomId, requestId ? `(requestId: ${requestId})` : '');
        // 채팅방 목록 새로고침
        await loadChatRooms();
  ...
  }, [chatAuth.isAuthenticated, chatAuth.token, loadChatRooms]);
  ...
  return {
    ...
    createChatRoom,
    findChatRoom,
    findChatRoomByRequestAndExpert,
    updateUnreadCount,
    updateLastMessage,
    updatePartnerStatus,
    refreshChatRooms,
