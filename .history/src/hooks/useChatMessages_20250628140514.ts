...
export const useChatMessages = (roomId: number | null) => {
    ...
    setMessages(prevMessages => {
      ...
      if (newMessage.messageId < 1000000000000) { // 서버 메시지 ID는 작은 숫자
        ...
        if (!newMessage.message || !newMessage.message.trim()) {
          ...
          // 서버 응답이 부정확할 수 있으므로 임시 메시지 중 가장 최근 것을 사용
          const tempMessages = prevMessages.filter(msg => 
            msg.messageId > 1000000000000 // 임시 ID만 체크
          );
          
          console.log('찾은 임시 메시지들:', tempMessages.map(msg => ({
          ...
          // 임시 메시지를 찾을 수 없으면 새 메시지를 시간순으로 추가
  ...
  // 내가 보낸 모든 안읽은 메시지를 읽음 처리 (상대방 입장 시)
  const markMyMessagesAsRead = useCallback(() => {
    setMessages(prevMessages => {
      const myUnreadMessages = prevMessages.filter(msg => 
        !msg.isRead && 
        msg.senderType === chatAuth.userType && 
        msg.senderId === chatAuth.userId
      );
      
      if (myUnreadMessages.length > 0) {
        console.log('상대방 입장으로 인한 내 메시지 읽음 처리:', myUnreadMessages.length, '개');
        
        return prevMessages.map(msg => 
          myUnreadMessages.some(unreadMsg => unreadMsg.messageId === msg.messageId)
            ? { ...msg, isRead: true }
            : msg
        );
      }
