...
export const useChatRooms = () => {
  ...
  // 채팅방 생성
  const createChatRoom = useCallback(async (expertId: string) => {
    ...
    try {
      chatApi.setAuthHeader(chatAuth.token);
      // customerId는 현재 로그인한 웹 사용자 ID, userId는 전문가 ID
      const response = await chatApi.createChatRoom(chatAuth.userId, expertId);

      if (response.success && response.data) {
        console.log('채팅방 생성 성공:', response.data.roomId);
        // 채팅방 목록 새로고침
        await loadChatRooms();
