...
export const useChatRooms = () => {
  ...
  // requestId로 채팅방 조회
  const findChatRoomByRequestId = useCallback(async (requestId: number) => {
    if (!chatAuth.isAuthenticated || !chatAuth.token) {
      throw new Error('인증이 필요합니다.');
    }

    try {
      chatApi.setAuthHeader(chatAuth.token);
      const response = await chatApi.findChatRoomByRequestId(requestId);

      if (response.success) {
        console.log('채팅방 조회 결과 (requestId):', response.data);
        return response.data?.roomId || null;
      } else {
        console.log('채팅방 조회 실패 (requestId):', response.error);
        return null;
      }
    } catch (error) {
      console.error('채팅방 조회 오류 (requestId):', error);
      return null;
    }
  }, [chatAuth.isAuthenticated, chatAuth.token]);

  // customerId와 userId로 채팅방 조회
  const findChatRoomByUsers = useCallback(async (customerId: number, userId: string) => {
    if (!chatAuth.isAuthenticated || !chatAuth.token) {
      throw new Error('인증이 필요합니다.');
    }
    ...
    try {
      chatApi.setAuthHeader(chatAuth.token);
      const response = await chatApi.findChatRoomByUsers(customerId, userId);

      if (response.success) {
        console.log('채팅방 조회 결과 (users):', response.data);
        return response.data?.roomId || null;
      } else {
        console.log('채팅방 조회 실패 (users):', response.error);
        return null;
      }
    } catch (error) {
      console.error('채팅방 조회 오류 (users):', error);
      return null;
    }
  }, [chatAuth.isAuthenticated, chatAuth.token]);
