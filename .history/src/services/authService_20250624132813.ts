...
import { ApiResponse, LoginRequest, LoginResponse, UserInfo, RegisterRequest } from '@/types/api';
...
export class AuthService {
  ...
  static async login(credentials: LoginRequest): Promise<ApiResponse<LoginResponse>> {
    ...
    if (response.success && response.data?.token && response.data?.userInfo) {
      AuthManager.setToken(response.data.token);
      AuthManager.setUserInfo(response.data.userInfo);
    }
