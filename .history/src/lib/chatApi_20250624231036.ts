...
class ChatApi {
  ...
  async getChatMessages(
    ...
    try {
      const response = await this.api.get(`/chat/${roomId}/messages`, {
        params: { page, limit }
      });
      return response.data;
    } catch (error) {
  ...
  // 채팅방 생성
  async createChatRoom(partnerUserId: string, partnerUserType: string): Promise<ChatApiResponse<{ roomId: number }>> {
    try {
  ...
  async updateUserStatus(status: "ONLINE" | "AWAY" | "BUSY" | "OFFLINE"): Promise<ChatApiResponse<void>> {
    try {
      const response = await this.api.post('/chat/status', { status });
      return response.data;
    } catch (error) {
  ...
  async markRoomMessagesAsRead(roomId: number): Promise<ChatApiResponse<void>> {
    try {
      const response = await this.api.post(`/chat/${roomId}/read`);
      return response.data;
    } catch (error) {
