...
class ChatApi {
  ...
  // 채팅방 생성
  async createChatRoom(requestId: number, customerId: string, expertId: string): Promise<ChatApiResponse<{ roomId: number }>> {
    try {
      const requestData = {
        requestId,
        customerId,
  ...
  async markMessagesAsRead(roomId: number, messageIds: number[]): Promise<ChatApiResponse<void>> {
    // API 호출 비활성화 - 일단 호출하지 않음
    console.log('메시지 읽음 처리 API 호출 비활성화됨:', { roomId, messageIds });
    return {
      success: true,
      message: '메시지 읽음 처리 API 비활성화됨'
    };
  }

  // 특정 채팅방의 읽지 않은 메시지 읽음 처리 (비활성화됨)
  async markRoomMessagesAsRead(roomId: number): Promise<ChatApiResponse<void>> {
    // API 호출 비활성화 - 일단 호출하지 않음
    console.log('채팅방 메시지 읽음 처리 API 호출 비활성화됨:', { roomId });
    return {
      success: true,
      message: '채팅방 메시지 읽음 처리 API 비활성화됨'
    };
  }
}
