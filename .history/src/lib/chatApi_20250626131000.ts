...
class ChatApi {
  ...
  constructor() {
    ...
    this.baseURL = process.env.NEXT_PUBLIC_CHAT_API_URL || 'https://www.tigerbk.com/chat-api/chat';
  ...
  async getChatMessages(
    ...
    try {
      const response = await this.api.get(`/${roomId}/messages`, {
        params: { page, limit }
      });
      return response.data;
    } catch (error) {
  ...
  async findChatRoomByRequestAndExpert(requestId: number, expertId: string): Promise<ChatApiResponse<{ roomId: number } | null>> {
    try {
      const response = await this.api.get('/find', {
  ...
  async createChatRoom(partnerUserId: string, partnerUserType: string, requestId?: number): Promise<ChatApiResponse<{ roomId: number }>> {
    try {
      ...
      const response = await this.api.post('/create', requestData);
      return response.data;
    } catch (error) {
  ...
  async updateUserStatus(status: "ONLINE" | "AWAY" | "BUSY" | "OFFLINE"): Promise<ChatApiResponse<void>> {
    try {
      const response = await this.api.post('/status', { status });
      return response.data;
    } catch (error) {
  ...
  async markMessagesAsRead(roomId: number, messageIds: number[]): Promise<ChatApiResponse<void>> {
    try {
      const response = await this.api.post('/messages/read', {
        roomId,
        messageIds
      });
      return response.data;
