...
class ChatApi {
  ...
  // customerId와 userId로 채팅방 조회 (대안)
  async findChatRoomByUsers(customerId: number, userId: string): Promise<ChatApiResponse<{ roomId: number } | null>> {
    try {
      const response = await this.api.get(`/chat/room/find`, {
      ...
      console.error('Find chat room by users API error:', error);
      throw error;
    }
  }
